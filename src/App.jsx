...
export default function App() {
  ...
  const [mGoal, setMGoal] = useState("General saving");
  ...
  const deletePlanned = async (p) => {
    setPlannedRows((r) => r.filter((row) =>
      !(normalizeMonth(row[0]) === p.month && String(row[1]).trim() === p.bucket && String(row[2]).trim() === p.description && Number(row[3]) === p.amount)));
  ...
  const monthlyEntries = useMemo(() => {
    ...
    const mi = h.indexOf("month"), ti = h.indexOf("type"), si = h.indexOf("description"), ai = h.indexOf("amount"), acci = h.indexOf("account"), gi = h.indexOf("goal");
    return monthlyRows.slice(hi + 1).map((r) => {
      const type = String(r[ti] || "").trim() || "Misc";
      let goal = gi >= 0 ? String(r[gi] || "").trim() : "";
      if (type === "Saved" && !goal) goal = "General saving"; // sensible default if left blank
      return {
        month: normalizeMonth(r[mi]),
        type,
        description: si >= 0 ? String(r[si] || "").trim() : "",
  ...
  const ledgerByMonth = useMemo(() => {
    return ledgerMonths.map((m) => {
      ...
      const general = saved.filter((e) => e.goal === "General saving");
      const trip = saved.filter((e) => e.goal === "Trip saving");
      const otherSaved = saved.filter((e) => e.goal !== "General saving" && e.goal !== "Trip saving");
      return {
        month: m, fixed, saved, misc, general, trip, otherSaved,
        fixedTotal: sum(fixed), savedTotal: sum(saved), miscTotal: sum(misc),
        generalTotal: sum(general), tripTotal: sum(trip), otherSavedTotal: sum(otherSaved),
      };
    });
  ...
  const addMonthlyEntry = async () => {
    ...
    const item = { month: mMonth, type: mType, description: mDesc.trim(), amount: amt, account: (mType === "Saved" || mType === "Income") ? mAccount : "", goal: mType === "Saved" ? mGoal : "" };
    setMonthlyRows((r) => [...r, [item.month, item.type, item.description, String(amt), item.account, item.goal]]);
    setMDesc(""); setMAmount("");
    showToast("Added " + money(amt) + " (" + mType + (item.goal ? " · " + item.goal : "") + ") for " + monthLabel(mMonth), null);
  ...
  const deleteMonthlyEntry = async (e) => {
    setMonthlyRows((r) => r.filter((row) =>
      !(normalizeMonth(row[0]) === e.month && String(row[1]).trim() === e.type && String(row[2]).trim() === e.description && Number(row[3]) === e.amount)));
  ...
  const posSummary = useMemo(() => {
    ...
    // General saving and Trip saving are now purely ledger-driven and
    // cumulative: every Saved entry tagged with that goal, from the start
    // through posMonth, keeps counting — savings don't reset each month the
    // way Fixed/Unplanned do. This replaces the old Budget-tab bucket formula
    // entirely, so there's only one place savings are tracked, not two.
    const cumulativeSaved = (goal) => monthlyEntries
      .filter((e) => e.type === "Saved" && e.goal === goal && e.month <= posMonth)
      .reduce((n, e) => n + e.amount, 0);
    const generalSaving = cumulativeSaved("General saving");
    const tripSaving = cumulativeSaved("Trip saving");
    ...
    // Spendable = what's free after this month's fixed bills and unplanned
    // spending, plus everything set aside for savings cumulatively so far.
    const spendable = projectedTotal - fixedTotal - unplannedTotal - generalSaving - tripSaving;
    return { fixedTotal, unplannedTotal, generalSaving, tripSaving, spendable, projectedTotal, monthsAhead };
  }, [ledgerByMonth, posMonth, positionTotal, monthlyEntries]);
    ...
    <div style={{ background: T.paper, minHeight: "100vh", color: T.ink, fontFamily: "'Work Sans', system-ui, sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 880px) { .cardgrid { grid-template-columns: repeat(2, 1fr); } .grid2 { grid-template-columns: 1fr; } .grid3 { grid-template-columns: 1fr; } .grid4 { grid-template-columns: repeat(2, 1fr); } .grid5 { grid-template-columns: repeat(3, 1fr); } }
        @media (max-width: 540px) { .cardgrid { grid-template-columns: 1fr; } .grid4 { grid-template-columns: 1fr; } .grid5 { grid-template-columns: repeat(2, 1fr); } .wrap { padding: 22px 14px 90px; } }
      `}</style>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                {mType === "Saved" && (
                  <select value={mGoal} onChange={(e) => setMGoal(e.target.value)}
                    style={{ border: `1px solid ${T.line}`, borderRadius: 10, padding: "9px 10px", fontSize: 13, minWidth: 140 }}>
                    <option value="General saving">General saving</option>
                    <option value="Trip saving">Trip saving</option>
                  </select>
                )}
            ...
            <div className="panel" style={{ marginBottom: 16, background: "#EFF5FA", borderColor: "#CBDFEE" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10, marginBottom: 14 }}>
                <div>
                  ...
                  {posSummary.projectedTotal !== positionTotal && (
                    <div style={{ fontSize: 11, color: T.muted, marginTop: 2 }}>
                      real balance
                      {posSummary.monthsAhead > 0 ? " + " + money(MONTHLY_INCOME) + " × " + posSummary.monthsAhead + " month" + (posSummary.monthsAhead === 1 ? "" : "s") : ""}
              ...
              <div className="grid5">
                ...
                <PosStat label="General saving" sub={"accrued by " + monthLabel(posMonth)} value={posSummary.generalSaving} color="#2E7D51" />
                <PosStat label="Trip saving" sub={"accrued by " + monthLabel(posMonth)} value={posSummary.tripSaving} color="#8A5A83" />
                <PosStat label="Spendable money" sub="after all four, right now" value={posSummary.spendable}
                  ...
                  <div key={g.month} className="panel">
                    ...
                    {g.fixed.length + g.saved.length + g.misc.length === 0 ? (
                      ...
                      <div className="grid4">
                        ...
                        <LedgerColumn label="General saving" color={T.green} items={g.general} total={g.generalTotal} onDelete={deleteMonthlyEntry} showAccount />
                        <LedgerColumn label="Trip saving" color="#8A5A83" items={g.trip} total={g.tripTotal} onDelete={deleteMonthlyEntry} showAccount />
